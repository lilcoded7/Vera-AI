...
class BMSAPIView(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        user = request.user 
        try:
            bms_sensor = BMSSENSOR.objects.get(user=user, timestamp=data.get('timestamp'))
        except BMSSENSOR.DoesNotExist:
            serializer = self.get_serializer(data=data)
            if serializer.is_valid():
                serializer.save(user=user)
                Response({'message': 'BMS Connected'}, status=status.HTTP_201_CREATED)
                return redirect('energy-prediction')
            else:
                return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
        else:
            serializer = self.get_serializer(bms_sensor, data=data)
            if serializer.is_valid():
                serializer.save(user=user)
                Response({'message': 'BMS Updated'}, status=status.HTTP_200_OK)
                return redirect('energy-prediction')
            else:
                return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
